...
func NewMainKubelet(kubeCfg *kubeletconfiginternal.KubeletConfiguration,
	...
	tracer := kubeDeps.TracerProvider.Tracer("")
	...
	klet.podManager = kubepod.NewBasicPodManager(mirrorPodClient, secretManager, configMapManager, tracer)

	klet.statusManager = status.NewManager(klet.kubeClient, klet.podManager, klet, tracer)
	...
	// setup containerGC
	containerGC, err := kubecontainer.NewContainerGC(klet.containerRuntime, containerGCPolicy, klet.sourcesReady, tracer)
	if err != nil {
		return nil, err
	}
	klet.containerGC = containerGC
	klet.containerDeletor = newPodContainerDeletor(klet.containerRuntime, integer.IntMax(containerGCPolicy.MaxPerPodContainer, minDeadContainerInPod), tracer)

	// setup imageManager
	imageManager, err := images.NewImageGCManager(klet.containerRuntime, klet.StatsProvider, kubeDeps.Recorder, nodeRef, imageGCPolicy, crOptions.PodSandboxImage, tracer)
	if err != nil {
...
func (kl *Kubelet) syncPod(ctx context.Context, updateType kubetypes.SyncPodType, pod, mirrorPod *v1.Pod, podStatus *kubecontainer.PodStatus) (isTerminal bool, err error) {
	...
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/syncPod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
	span.SetAttributes(attribute.String("updateType", updateType.String()))
	...
	apiPodStatus := kl.generateAPIPodStatus(ctx, pod, podStatus)
	...
	// since kubelet first saw the pod if firstSeenTime is set.
	existingStatus, ok := kl.statusManager.GetPodStatus(ctx, pod.UID)
	...
	if !kl.podWorkers.IsPodTerminationRequested(ctx, pod.UID) {
		if kl.secretManager != nil {
			kl.secretManager.RegisterPod(ctx, pod)
		}
		if kl.configMapManager != nil {
			kl.configMapManager.RegisterPod(ctx, pod)
		}
	}
	...
	if err := kl.makePodDataDirs(ctx, pod); err != nil {
...
func (kl *Kubelet) syncTerminatingPod(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus, runningPod *kubecontainer.Pod, gracePeriod *int64, podStatusFn func(*v1.PodStatus)) error {
	...
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/syncTerminatingPod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
...
func (kl *Kubelet) syncTerminatedPod(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus) error {
	...
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/syncTerminatedPod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
...
func (kl *Kubelet) deletePod(ctx context.Context, pod *v1.Pod) error {
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/deletePod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
	if pod == nil {
...
func (kl *Kubelet) rejectPod(ctx context.Context, pod *v1.Pod, reason, message string) {
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/rejectPod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
...
func (kl *Kubelet) canRunPod(ctx context.Context, pod *v1.Pod) lifecycle.PodAdmitResult {
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/canRunPod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
...
func (kl *Kubelet) dispatchWork(ctx context.Context, pod *v1.Pod, syncType kubetypes.SyncPodType, mirrorPod *v1.Pod, start time.Time) {
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/dispatchWork")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
...
func (kl *Kubelet) handleMirrorPod(ctx context.Context, mirrorPod *v1.Pod, start time.Time) {
	ctx, span := kl.tracer.Start(ctx, "pkg.kubelet.kubelet/handleMirrorPod")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", mirrorPod.Name))
	span.SetAttributes(attribute.String("pod.namespace", mirrorPod.Namespace))
	// Mirror pod ADD/UPDATE/DELETE operations are considered an UPDATE to the
	// corresponding static pod. Send update to the pod worker if the static
	// pod exists.
	if pod, ok := kl.podManager.GetPodByMirrorPod(mirrorPod); ok {
		kl.dispatchWork(ctx, pod, kubetypes.SyncPodUpdate, mirrorPod, start)
	}
}
...
func (kl *Kubelet) HandlePodAdditions(pods []*v1.Pod) {
	ctx, span := kl.tracer.Start(context.TODO(), "pkg.kubelet.kubelet/HandlePodAdditions")
	defer span.End()
	start := kl.clock.Now()
	...
	for _, pod := range pods {
		...
		kl.podManager.AddPod(ctx, pod)

		if kubetypes.IsMirrorPod(pod) {
			kl.handleMirrorPod(ctx, pod, start)
			continue
		}
...
func (kl *Kubelet) HandlePodUpdates(pods []*v1.Pod) {
	ctx, span := kl.tracer.Start(context.TODO(), "pkg.kubelet.kubelet/HandlePodUpdates")
	defer span.End()
	start := kl.clock.Now()
	for _, pod := range pods {
		kl.podManager.UpdatePod(ctx, pod)
		if kubetypes.IsMirrorPod(pod) {
			kl.handleMirrorPod(ctx, pod, start)
			continue
		}
...
func (kl *Kubelet) HandlePodRemoves(pods []*v1.Pod) {
	ctx, span := kl.tracer.Start(context.TODO(), "pkg.kubelet.kubelet/HandlePodRemoves")
	defer span.End()
	start := kl.clock.Now()
	for _, pod := range pods {
		kl.podManager.DeletePod(ctx, pod)
		if kubetypes.IsMirrorPod(pod) {
			kl.handleMirrorPod(ctx, pod, start)
			continue
		}
...
func (kl *Kubelet) HandlePodReconcile(pods []*v1.Pod) {
	ctx, span := kl.tracer.Start(context.TODO(), "pkg.kubelet.kubelet/HandlePodReconcile")
	defer span.End()
	start := kl.clock.Now()
	for _, pod := range pods {
		...
		if status.NeedToReconcilePodReadiness(pod) {
			mirrorPod, _ := kl.podManager.GetMirrorPodByPod(pod)
			kl.dispatchWork(ctx, pod, kubetypes.SyncPodSync, mirrorPod, start)
		}
...
func (kl *Kubelet) cleanUpContainersInPod(podID types.UID, exitedContainerID string) {
	ctx, span := kl.tracer.Start(context.TODO(), "pkg.kubelet.kubelet/cleanUpContainersInPod")
	defer span.End()
