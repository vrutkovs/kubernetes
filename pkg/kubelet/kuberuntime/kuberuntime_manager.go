...
func (m *kubeGenericRuntimeManager) computePodActions(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus) podActions {
	...
	ctx, span := m.tracer.Start(ctx, "pkg.kubelet.kuberuntime.kuberuntime_manager/computePodActions")
	defer span.End()
...
func (m *kubeGenericRuntimeManager) killPodWithSyncResult(ctx context.Context, pod *v1.Pod, runningPod kubecontainer.Pod, gracePeriodOverride *int64) (result kubecontainer.PodSyncResult) {
	ctx, span := m.tracer.Start(ctx, "pkg.kubelet.kuberuntime.kuberuntime_manager/killPodWithSyncResult")
	defer span.End()
	span.SetAttributes(attribute.String("pod.name", pod.Name))
	span.SetAttributes(attribute.String("pod.namespace", pod.Namespace))
